...
function getAverageRating(watchList, director) {
	const directorFilms = watchList.filter(
		({ Director }) => Director === director
	)
	const filmRatings = directorFilms.map((film) => Number(film.imdbRating))
	const summedFilmRatings = filmRatings.reduce((sum, rating) => rating + sum, 0)
	const averageRating = summedFilmRatings / directorFilms.length
	return averageRating
}
